...
void Character::deserialize(std::istream& is)
{
    ...
    // Skip stored ability_uses value
    int stored_ability_uses;
    is.read(reinterpret_cast<char*>(&stored_ability_uses), sizeof(stored_ability_uses));

    // Set ability_uses to character's level
    ability_uses = lvl;
